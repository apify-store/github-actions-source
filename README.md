...
| `TESTER_APIFY_TOKEN_READ_ONLY` | the Claude step of `platform-tests-claude-investigate-and-fix`, where it authenticates the Apify MCP server so the investigation can read the failing run. Used only for read-only investigation. |
